...
import { getOrders, getCustomers, getItems, getCategories, getTables } from "@/shared/api/services/dashboardApi";
...
const MetricCard: React.FC<MetricCardProps> = ({ title, value, percentage, icon, color, isIncrease, delay }) => (
  <motion.div 
...
const Metrics: React.FC<MetricsProps> = ({ branchId = "all" }) => {
  ...
  const ordersList = fullOrdersList.filter((order: any) => {
