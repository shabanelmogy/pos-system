...
export const orderPaymentService = {
  async addPayment(orderId, paymentData, context) {
    ...
    return await db.transaction(async (tx) => {
      ...
      await paymentRepository.create({
        orderId, amount: paymentAmount.toFixed(2), method, reference: paymentId,
  ...
  async refundOrder(orderId, reason, context) {
    const { userId, role, branchId } = context;
  ...
  async updatePaymentStatus(orderId, newStatus, actorId, tx) {
    const order = await orderRepository.findByIdWithLock(orderId, tx);
    if (!order) fail("Order not found", 404);
