...
import { getOrders, updateOrderLifecycle, updateOrderFulfillment } from "@/shared/api/services/dashboardApi";
            ...
            {(filteredOrders || []).map((order: any, index: number) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={cell}>
                    <select
                      className={`h-5 w-full min-w-0 max-w-full rounded-md border border-[var(--border-main)] bg-[var(--bg-card)] px-px py-0 text-[8px] leading-none text-[var(--text-main)] focus:outline-none 2xl:h-6 2xl:rounded-lg 2xl:px-1 2xl:py-0.5 2xl:text-[10px] ${order.lifecycle === "COMPLETED"
                          ? "text-[var(--primary)]"
                  ...
                  <td className={`${cell} text-center`}>
                    <span
                      className={`inline-block max-w-full truncate rounded-full px-0.5 py-px text-[6px] font-bold uppercase 2xl:px-1 2xl:text-[8px] ${order.paymentStatus === "PAID"
