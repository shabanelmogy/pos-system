    ...
    <div className="mt-4 mb-6 relative">
      <div className="bg-[var(--bg-card)] w-full min-h-[300px] lg:h-[331px] 2xl:h-[503px] rounded-2xl border border-[var(--border-main)] shadow-xl overflow-hidden flex flex-col">
        <div className="flex justify-between items-center px-6 py-4 border-b border-[var(--bg-card-alt)]">
          <h1 className="text-[var(--text-main)] text-base font-black uppercase tracking-tighter">{t('pos.home.recent_orders')}</h1>
          <button className="text-[var(--primary)] text-[9px] font-black uppercase tracking-widest hover:underline">{t('pos.home.view_history')}</button>
        </div>
        ...
        <div className="overflow-y-auto min-h-[300px] lg:h-[420px] scrollbar-hide">
          <div className="flex items-center gap-3 bg-[var(--bg-card-alt)] rounded-xl px-4 py-2.5 mb-4 border border-[var(--border-main)] focus-within:border-[var(--primary)] transition-colors mx-6 mt-4">
            ...
            <input type="text" placeholder={t('pos.home.search_history')}
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar pe-2 px-6">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center h-full gap-4 opacity-50">
                ...
                <p className="text-[10px] text-[var(--text-muted)] font-black uppercase tracking-widest">{t('pos.home.fetching')}</p>
              </div>
            ) : ordersList.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center py-10">
                <p className="text-[var(--text-dim)] font-black uppercase tracking-widest text-xs">{t('pos.home.no_recent')}</p>
                <p className="text-[var(--border-main)] text-[10px] mt-1 font-bold uppercase">{t('pos.home.ready_for_first')}</p>
              </div>
            )}
          </div>
        </div>
