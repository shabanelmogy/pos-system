  ...
  return (
    <div 
      ...
      className={`w-full hover:bg-[var(--bg-hover)] bg-[var(--bg-card-alt)] p-4 rounded-xl cursor-pointer transition-all border-2 ${
        status === "Occupied" ? "border-[var(--status-success)]" : "border-transparent"
      }`}
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
           ...
           <p className={`text-[10px] uppercase font-bold px-2 py-1 rounded ${
             status === "Occupied" ? "bg-[var(--status-success-bg)] text-[var(--status-success)]" : "bg-[var(--border-main)] text-[var(--text-muted)]"
           }`}>
             {status}
           </p>
        </div>
