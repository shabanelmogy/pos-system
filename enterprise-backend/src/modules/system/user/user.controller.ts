...
import shiftRepository from "../../pos/shift/shift.repository.js";
            ...
            const accessToken = jwt.sign(
                {
                    ...
                    userId: user.id,
                    email: user.email,
                    roles: rbacData.roles.map(r => r.name),
                    permissions: rbacData.permissions.map(p => p.key),
                    permissionsVersion: user.permissionsVersion
                },
                config.accessTokenSecret!,
                {
                    expiresIn: '1d' // guide recommendation
                }
            );
                ...
                const accessToken = jwt.sign(
                    {
                        ...
                        roles: rbacData.roles.map(r => r.name),
                        permissions: rbacData.permissions.map(p => p.key),
                        permissionsVersion: latestUser.permissionsVersion
                    },
                    config.accessTokenSecret!,
                    {
                        expiresIn: '1d'
