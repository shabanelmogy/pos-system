...
export const orderItemService = {
  async addItem(orderId, itemData, context) {
    ...
    return await db.transaction(async (tx) => {
      ...
      if (reqModifiers?.length > 0) {
        const modIds = reqModifiers.map(m => m.modifierId);
  ...
  async updateItemQuantity(orderId, itemId, newQuantity, context) {
  ...
  async removeItem(orderId, itemId, context) {
    ...
    return await db.transaction(async (tx) => {
      ...
      // The returned `updated` object carries the post-recalculate version, which must be
      // threaded into any subsequent recordStatusChange call. Do not use order.version here.
  ...
  async voidItem(orderId, itemId, reason, context) {
    const { userId, role, branchId } = context;
    ...
    return await db.transaction(async (tx) => {
      ...
      // NOTE: see removeItem — updated.version must be used for any subsequent
      // recordStatusChange, not the pre-recalculate order.version.
      const updated = await orderBaseService._recalculateTotals(orderId, tx);

      // grandTotal snapshot is intentionally taken before _recalculateTotals — it
      // records "what was the total at the moment this item was voided", consistent
      // with how other audit history snapshots work. Do not move this after recalculate.
      await orderRepository.recordStatusChange({
