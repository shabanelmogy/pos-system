...
import { MdTableBar, MdCategory, MdAddCircleOutline, MdSpaceDashboard } from "react-icons/md";
...
const Dashboard = () => {
  ...
  return (
    <div className="bg-[#121212] min-h-[calc(100vh-5rem)] pb-20 overflow-y-auto">
      {/* Top Banner / Hero */}
      <div className="bg-gradient-to-r from-[#1a1a1a] to-[#262626] border-b border-[#333]">
          ...
          <div className="flex items-center gap-3 flex-wrap justify-center">
            {canManageSettings && buttons.map(({ label, icon, action, color }) => (
              <motion.button
                ...
                className={`bg-gradient-to-br ${color} px-6 py-3 rounded-xl text-white font-bold text-sm flex items-center gap-2 shadow-lg hover:shadow-xl transition-all border border-white/10`}
              >
      ...
      <div className="container mx-auto px-6 mt-10">
        <div className="flex items-center bg-[#1a1a1a] p-1.5 rounded-2xl border border-[#333] w-fit">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              className={`
                px-8 py-3 rounded-xl font-bold text-sm flex items-center gap-2 transition-all duration-300 ${
                activeTab === tab.id
