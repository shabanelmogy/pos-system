...
async function migrate(): Promise<void> {
  ...
  try {
    ...
    const allOrders = await db.select().from(orders).where(isNotNull((orders as any).customerSnapshot));
    console.log(`Analyzing ${allOrders.length} orders for customer data.`);

    const customerMap = new Map<string, CustomerMapEntry>(); // phone -> { name, spent, count, lastDate, id }

    for (const order of allOrders) {
      const snap = (order as any).customerSnapshot;
      if (!snap || !snap.phone) continue;

      const phone: string = snap.phone;
      const amount: number = parseFloat(((order as any).total as string) || "0");
      const orderDate: any = order.createdAt;
