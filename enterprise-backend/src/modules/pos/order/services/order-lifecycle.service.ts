...
import itemRepository from "../../../catalog/item/item.repository.js";
...
export const orderLifecycleService = {
  ...
  async createOrder(data: any, context: any): Promise<any> {
    ...
    return await db.transaction(async (tx) => {
      ...
      const menuItemIds: string[] = [...new Set(requestItems.map((i: any) => i.menuItemId))] as string[];
      const products = await itemRepository.findByIds(menuItemIds);
      const productMap = new Map(products.map((p: any) => [p.id, p]));

      const modifierIds: string[] = [...new Set(requestItems.flatMap((i: any) => i.modifiers || []).map((m: any) => m.modifierId))] as string[];
  ...
  async confirmDraft(id: string, context: ServiceContext): Promise<any> {
  ...
  async updateFulfillmentStatus(id: string, status: string, context: ServiceContext, notes?: string, reasonCode?: string): Promise<any> {
    const { userId, branchId, role } = context;
    return await db.transaction(async (tx) => {
      const order = await orderRepository.findByIdWithLock(id, tx);
      if (!order) fail("order.not_found", 404);
  ...
  async updateLifecycle(id: string, status: string, context: ServiceContext, notes?: string, reasonCode?: string, options: any = {}): Promise<any> {
    const { userId, branchId, role } = context;
    ...
    const { updatedOrder, tableId } = await db.transaction(async (tx) => {
      ...
      if (status === "COMPLETED") {
        ...
        if (balance.gt(0)) {
          ...
          logger.error(`Data integrity error: zero balance but paymentStatus is not PAID for order ${id}, status: ${order.paymentStatus}`);
    ...
    // Emit socket events AFTER the transaction commits
    setImmediate(() => {
