...
export const TYPE_FULFILLMENT_GUARDS: Record<string, string[]> = {
...
export const orderBaseService = {
  _validatePaymentTransition(from: string, to: string): void {
  ...
  _guardAgainstPaidModification(order: any, role: string): void {
  ...
  async _generateOrderNumber(branchId: string, tx: any): Promise<string> {
    const now = new Date();
  ...
  async _createOrderWithBill(orderData: any, itemsWithModifiers: any[], tx: any): Promise<any> {
  ...
  async _recalculateTotals(orderId: string, tx: any): Promise<any> {
    ...
    let couponDiscountAmount: string | null = null;
    ...
    const totals = pricingService.calcOrderTotals(totalSubtotal, totalDiscount, taxRate, serviceChargeRate, couponDiscountAmount) as any;
